...
- Please verify and extra user *atlas* is added to FortiSOAR user, similar way you can add your own  user and assign required email id.
...
- *FortiGate Firewall Configurations*
  * Open FortiGate GUI  with admin privilages
  * Create an address group: Allowed_LAN_Devices if it doesn’t exist already
  * Create an SNAT policy to allow traffic from Allowed_LAN_Devices to the internet if it doesn’t exist already
  * Create a Web Filter Profile named *FortiSOAR* enabling URL Filter setting
  * Create an Application Control Profile named *FortiSOAR* with default settings
